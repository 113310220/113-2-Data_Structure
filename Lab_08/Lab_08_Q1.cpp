...
private:
    ...
    TreeNode* deleteNode(TreeNode* node, int val) {
        ...
        if (node->left == nullptr && node->right == nullptr) {//兩邊為空時
            delete node;//刪除元素
            return nullptr;//回傳為空
        } 
        else if (node->left == nullptr) {//左邊為空，右邊為空時
            TreeNode* temp = node->right;//找右邊頂替
            delete node;//刪除元素
            return temp;
        } 
        else if (node->right == nullptr) {//右邊為空，左邊為空時
            TreeNode* temp = node->left;//找左邊頂替
            delete node;//刪除元素
            return temp;
        } 
        else {//兩邊皆不為空時
            
            TreeNode* temp = findMin(node->right);//找尋右邊(較大)的數
            node->data = temp->data;//頂替
            node->right = deleteNode(node->right, temp->data);//刪除元素
        }
    }
     return node;
}
