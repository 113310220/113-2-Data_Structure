...
void findMax(TreeNode* node, int& maxVal) {
    ...
    findMax(node->left, maxVal);
    findMax(node->right, maxVal);
}
...
int main() {
    ...
    if (tree.root != nullptr) {
        int leftMax = tree.root->left ? tree.root->left->value : -1;
        int rightMax = tree.root->right ? tree.root->right->value : -1;

        if (tree.root->left) {
            tree.findMax(tree.root->left, leftMax);  
        }

        if (tree.root->right) {
            tree.findMax(tree.root->right, rightMax);  
        }
